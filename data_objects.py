...
class heatmap_object(object):
    ...
    def map_(self, data, keys):
        ...
        for id_ in mapped:
            ...
            tcell = air_temperature + \
                (module_globals['gamma'] * (solar_radiation / 1000))
            #tcell = air_temperature + (float(self.pv['rated_tcell'] - 20)/0.8) * solar_radiation/1000
            voc = self.pv['rated_voc'] * \
                (1 - 0.0037 * (tcell - self.pv['rated_tcell']))
            pmax = self.pv['rated_power'] * \
                (1 - 0.005 * (tcell - self.pv['rated_tcell']))
            isc = self.pv['rated_isc'] * \
                (1 - 0.0037 * (tcell - self.pv['rated_tcell']))
    ...
    def map_(self, data, keys):
        ...
        for k, v in mapped.iteritems():
            ...
            for sr, at in zip(solar_radiation, air_temperature):
                tcell = round(
                    at + (module_globals['gamma'] * (float(sr) / 1000)), 2)
                voc = round(
                    self.pv['rated_voc'] * (1 - 0.0037 * (tcell - self.pv['rated_tcell'])), 2)
                pmax = round(
                    self.pv['rated_power'] * (1 - 0.005 * (tcell - self.pv['rated_tcell'])), 2)
                isc = round(
                    self.pv['rated_isc'] * (1 - 0.0037 * (tcell - self.pv['rated_tcell'])), 2)
                voc_.append(voc)
                pmax_.append(pmax)
                isc_.append(isc)
                tcell_.append(tcell)
